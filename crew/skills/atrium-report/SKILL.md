# Atrium Pipeline Report Generator

## Metadata
- **Title:** Sales Pipeline Report Generator
- **Invocation:** `/atrium report`
...
When the user invokes `/atrium report`, follow this process:
...
1. **Analyze a prospect:** Run `/atrium prospect <company-website-url>` to analyze a potential customer
2. **Build your ICP first (recommended):** Run `/atrium icp <description>` to define your ideal customer profile
3. **Analyze multiple prospects:** Run the prospect command for each company you're evaluating
4. **Generate this report:** Run `/atrium report` again after analyzing at least one prospect

### Example Workflow

```
/atrium icp "We sell an API monitoring platform for $500-2000/mo to mid-market SaaS companies"
/atrium prospect https://company1.com
/atrium prospect https://company2.com
/atrium prospect https://company3.com
/atrium report
```
```
...
Provide guidance:
- Which prospects need outreach sequences created (suggest running `/atrium prospect <url>` if missing)
...
Weights below come from `crew/config/qualify.config.json` — read them from
config at report generation time, never hardcode them here:

- Company Fit: `weights.companyFit` of score, currently 25% (firmographics, tech stack, growth signals)
- Contact Access: `weights.contactAccess` of score, currently 20% (decision makers, personalization, warm paths)
- Opportunity Quality: `weights.opportunityQuality` of score, currently 20% (BANT qualification, pain severity, timeline)
- Competitive Position: `weights.competitivePosition` of score, currently 15% (current tools, switching costs, advantages)
- Outreach Readiness: `weights.outreachReadiness` of score, currently 20% (personalization, channel strategy, messaging)

*Report generated by Atrium | Refresh by running `/atrium report`*
```
