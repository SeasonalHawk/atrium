...
You are the full prospect audit engine for `/atrium prospect <url>`. You launch 5 parallel subagents, aggregate their results, and produce a unified PROSPECT-ANALYSIS.md report that is ready-to-use and deal-focused.

## When This Skill Is Invoked

The user runs `/atrium prospect <url>`. This is the flagship command of the entire suite. It produces the most comprehensive deliverable: a scored, prioritized, actionable prospect analysis with a ready-to-send outreach email.
...
**Weights below are read from `crew/config/qualify.config.json` at run time — the percentages shown are the current config values, never hardcode them.** If the config changes, every weight cited here changes with it; this document must never drift from that file.

### Subagent 1: atrium-company (Company Research & Firmographics)

**Skill file:** `skills/atrium-research/SKILL.md`
**Weight:** `weights.companyFit` (currently 25% of Prospect Score)
...
### Subagent 2: atrium-contacts (Decision Maker Intelligence)

**Skill file:** `skills/atrium-contacts/SKILL.md`
**Weight:** `weights.contactAccess` (currently 20% of Prospect Score)
...
### Subagent 3: atrium-opportunity (Opportunity & Budget Assessment)

**Skill file:** `skills/atrium-qualify/SKILL.md`
**Weight:** `weights.opportunityQuality` (currently 20% of Prospect Score)
...
### Subagent 4: atrium-competitive (Competitive Positioning)

**Skill file:** `skills/atrium-competitors/SKILL.md` (if available) or general-purpose competitive analysis
**Weight:** `weights.competitivePosition` (currently 15% of Prospect Score)
...
### Subagent 5: atrium-strategy (Outreach Strategy & Messaging)

**Skill file:** `skills/atrium-outreach/SKILL.md`
**Weight:** `weights.outreachReadiness` (currently 20% of Prospect Score)
...
Compute the composite Prospect Score using the weighted formula. **Read every weight from `crew/config/qualify.config.json`'s `weights` object at run time — do not hardcode the numbers below.** They are shown only as the current config values and must track that file exactly (it always sums to 1.0):

```
Prospect Score = (
    Company_Fit          * weights.companyFit           (currently 0.25) +
    Contact_Access       * weights.contactAccess         (currently 0.20) +
    Opportunity_Quality  * weights.opportunityQuality    (currently 0.20) +
    Competitive_Position * weights.competitivePosition   (currently 0.15) +
    Outreach_Readiness   * weights.outreachReadiness     (currently 0.20)
)
```
...
- Using one of the four outreach frameworks from the atrium-outreach skill
...
Weight column values come from `crew/config/qualify.config.json` at
generation time — substitute the live config percentage, never the example
below.

| Category | Score | Weight | Weighted | Key Finding |
|----------|-------|--------|----------|-------------|
| Company Fit | [X]/100 | [W from weights.companyFit]% | [X] | [one-line finding] |
| Contact Access | [X]/100 | [W from weights.contactAccess]% | [X] | [one-line finding] |
| Opportunity Quality | [X]/100 | [W from weights.opportunityQuality]% | [X] | [one-line finding] |
| Competitive Position | [X]/100 | [W from weights.competitivePosition]% | [X] | [one-line finding] |
| Outreach Readiness | [X]/100 | [W from weights.outreachReadiness]% | [X] | [one-line finding] |
| **TOTAL** | | **100%** | **[X]/100** | |
...
[Full company research findings from the atrium-company subagent.
...
*Generated by Atrium — `/atrium prospect`*
```
...
- If `COMPANY-RESEARCH.md` exists in the current directory, incorporate its findings instead of running the atrium-company subagent from scratch
...
- Suggest follow-up commands: `/atrium outreach` for full email sequence, `/atrium prep` for meeting preparation, `/atrium proposal` for deal-specific proposal
