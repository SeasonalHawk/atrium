# Professional Atrium Report PDF Generator

## Metadata
- **Title:** Professional Sales Report PDF Generator
- **Invocation:** `/atrium report-pdf`
...
When the user invokes `/atrium report-pdf`, follow this process:
...
- Inform the user: "No SALES-REPORT.md found. Run `/atrium report` first to generate the pipeline report, then run `/atrium report-pdf` to create the PDF version."
- Stop execution.
...
1. The atrium (adapted from ai-atrium-team-claude) project directory (where the agents/ and skills/ folders are)
2. The current working directory
3. One level up from the current working directory

**If the script does NOT exist:**
- Inform the user: "The PDF generation script was not found at `scripts/generate_pdf_report.py`. This script is part of the Atrium project setup. Please ensure the project is properly installed."
- Stop execution.
...
This script is part of the Atrium project. Please ensure the project
...
Run `/atrium report` first to generate the pipeline report, then run
`/atrium report-pdf` to create the PDF version.
```
